...
dplot_adist <- function(
  ...
  if (type == "bar") {
    ggplot2::ggplot(
      x,
      ggplot2::aes(x = .data$Age, y = .data$value, fill = .data$year_class)
    ) +
      ggplot2::geom_vline(
        xintercept = age_groups,
        color = "grey",
        linewidth = 0.5 / 2.13
      ) +
      ggplot2::geom_vline(
        xintercept = attr(first_age_group[[1]], "min"),
        color = "grey",
        linetype = ifelse(
          !is.null(attr(first_age_group[[1]], "min_open_ended")),
          "dotted",
          "solid"
        ),
        linewidth = 1 / 2.13
      ...
      ggplot2::geom_vline(
        xintercept = attr(last_age_group[[1]], "max"),
        color = "grey",
        linetype = ifelse(
          attr(last_age_group[[1]], "max_open_ended"),
          "dotted",
          "solid"
        ),
        linewidth = 1 / 2.13
      ...
      ggplot2::geom_col() +
      ggplot2::facet_wrap(~year, dir = dir, ...) +
