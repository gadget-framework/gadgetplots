...
```{r mortality}
  plotly::ggplotly(plot_hr(fit, min_catch_length = 45))
```
...
```{r recruitment}
plotly::ggplotly(gadgetplots::plot_ageclass(fit) + 
...
if(inherits(fit$params, "list")) {
  tmp <- stats::setNames(as.data.frame(fit$params), "value") %>% 
    dplyr::mutate(switch = names(fit$params), .before = 1)
} else {
  tmp <- fit$params 
}
...
DT::datatable(
  tmp %>% 
    dplyr::filter(
      grepl("K$|Linf|recl$|M$|rec.sd|init.F|bbin|_l50$|_alpha|_a50$", 
            .data$switch)) %>% 
...
```{r allparams}
DT::datatable(tmp, options = list(pageLength = 500)) %>%
  DT::formatRound(columns=c('value'), digits=3)
```
